...
require(__PANEL__ . '/ReviewEditPanel.class.php');

/**
 * This is a draft QForm object to do Create, Edit, and Delete functionality
 * of the Review class.  It uses the code-generated
 * ReviewConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of Review columns.
 *
 ...
 * @subpackage Drafts
 */
class ReviewEditForm extends QForm {
...
//	protected function Form_Load() {}
	...
	protected function Form_Create() {
        ...
        $this->strObjectName = QApplication::Translate('Review');
        $this->strObjectNamePlural = QApplication::Translate('Reviews');

        $this->pnlReview = new ReviewEditPanel($this);
		$intId = QApplication::QueryString('intId');
	    $this->pnlReview->Load($intId);
		$this->CreateButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Review'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->pnlReview->mctReview->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlReview->Save(true);
            $this->GetControl($strControlId)->Close();
            $this->RedirectToListPage();
        } else { // Refresh
            $this->GetControl($strControlId)->Close();
            $this->pnlReview->Refresh(true);
        }
    }
    ...
    */
	protected function RedirectToListPage() {
		QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORMS__ . '/review_list.php',
...
// review_edit.tpl.php as the included HTML template file
ReviewEditForm::Run('ReviewEditForm');
