...
require(__PANEL__ . '/TypeEditPanel.class.php');

/**
 * This is a draft QForm object to do Create, Edit, and Delete functionality
 * of the Type class.  It uses the code-generated
 * TypeConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of Type columns.
 *
 ...
 * @subpackage Drafts
 */
class TypeEditForm extends QForm {
...
//	protected function Form_Load() {}
	...
	protected function Form_Create() {
        ...
        $this->strObjectName = QApplication::Translate('Type');
        $this->strObjectNamePlural = QApplication::Translate('Types');

        $this->pnlType = new TypeEditPanel($this);
		$strName = QApplication::QueryString('strName');
	    $this->pnlType->Load($strName);
		$this->CreateButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Type'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->pnlType->mctType->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlType->Save(true);
            $this->GetControl($strControlId)->Close();
            $this->RedirectToListPage();
        } else { // Refresh
            $this->GetControl($strControlId)->Close();
            $this->pnlType->Refresh(true);
        }
    }
    ...
    */
	protected function RedirectToListPage() {
		QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORMS__ . '/type_list.php',
...
// type_edit.tpl.php as the included HTML template file
TypeEditForm::Run('TypeEditForm');
