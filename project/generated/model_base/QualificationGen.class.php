<?php
	 ...
	 * @property string $Name the value of the name column (Unique)
	 ...
	 */
	class QualificationGen extends QBaseClass implements IteratorAggregate, JsonSerializable {
		...
		/** @var Qualification[] Short term cached Qualification objects */
	...
	///////////////////////////////////////////////////////////////////////
		...
		/**
		 * Protected member variable that maps to the database PK Identity column qualification.id
		 * @var integer intId
		 */
		private $intId;
		...
		/**
		 * Protected member variable that maps to the database column qualification.description
		...
		/**
		 * Protected member variable that maps to the database column qualification.active
		...
		/**
		 * Construct a new Qualification object.
		 */
		...
		{
			$this->strName = Qualification::NameDefault;
			$this->__blnValid[self::NAME_FIELD] = true;
			$this->strDescription = Qualification::DescriptionDefault;
			$this->__blnValid[self::DESCRIPTION_FIELD] = true;
			$this->blnActive = Qualification::ActiveDefault;
		...
		**/
		...
		public static function Load($intId, $objOptionalClauses = null) {
			...
			$objToReturn = Qualification::QuerySingle(
				QQ::AndCondition(
					QQ::Equal(QQN::Qualification()->Id, $intId)
				),
				$objOptionalClauses
			);
			return $objToReturn;
		...
		/**
		 * Static Qcubed Query method to query for a single Qualification object.
		...
		/**
		 * Static Qcubed Query method to query for an array of Qualification objects.
		 * Offloads work to QModelTrait.trait.php
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClausees additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		...
		public static function QueryPrimaryKeys(QQCondition $objConditions = null) {
			...
			$clauses[] = QQ::Select(QQN::Qualification()->Id);
			$objQualifications = self::QueryArray($objConditions, $clauses);
			$pks = [];
			foreach ($objQualifications as $objQualification) {
				$pks[] = $objQualification->intId;
			}
			return $pks;
		}
		 ...
		 * @return mixed Either a Qualification, or false to indicate the dbrow was used in an expansion, or null to indicate that this leaf is a duplicate.
		*/
		public static function InstantiateDbRow(
			...
			if (empty($objToReturn)) {
				...
				$objToReturn = new Qualification(false);
		...
		public static function InstantiateDbResult(QDatabaseResultBase $objDbResult, $objExpandAsArrayNode = null, $strColumnAliasArray = null) {
			...
			if ($objExpandAsArrayNode) {
				...
				while ($objDbRow = $objDbResult->GetNextRow()) {
					$objItem = Qualification::InstantiateDbRow($objDbRow, null, $objExpandAsArrayNode, $objPrevItemArray, $strColumnAliasArray);
					if ($objItem) {
						$objToReturn[] = $objItem;
		...
		public static function InstantiateCursor(QDatabaseResultBase $objDbResult) {
			...
			return Qualification::InstantiateDbRow($objDbRow, null, null, null, $strColumnAliasArray);
		}
		...
		*/
		public static function LoadByName($strName, $objOptionalClauses = null) {
			return Qualification::QuerySingle(
				QQ::AndCondition(
					QQ::Equal(QQN::Qualification()->Name, $strName)
				),
				$objOptionalClauses
			);
		}
	...
	*/
	protected function Insert() {
		...
		$mixToReturn = $this->intId = $objDatabase->InsertId('qualification', 'id');
	...
	*/
	...
	public function Reload($clauses = null) {
		...
		// Reload the Object
		$objReloaded = Qualification::Load($this->intId, $clauses);
		...
		/**
		 *  Return an array of Qualifications keyed by the unique Id property.
		 *	@param Qualification[]
		 *	@return Qualification[]
		 **/
		public static function KeyQualificationsById($qualifications) {
			if (empty($qualifications)) {
				return $qualifications;
			}
			foreach ($qualifications as $qualification) {
				$aOut[$qualification->intId] = $qualification;
			}
			return $aOut;
		}
		...
		/**
		 *  Return an array of Qualifications keyed by the unique Name property.
		 *	@param Qualification[]
		 *	@return Qualification[]
		 **/
		public static function KeyQualificationsByName($qualifications) {
			if (empty($qualifications)) {
				return $qualifications;
			}
			foreach ($qualifications as $qualification) {
				$aOut[$qualification->strName] = $qualification;
			}
			return $aOut;
		}
...
//////////////////////////////////////////////////////////////
   ...
   /**
	* Gets the value of strName (Unique)
   ...
   /**
	* Sets the value of strName (Unique)
	...
	*/
	protected static function BroadcastInsert($pk) {
		if (static::$blnWatchChanges) {
			QWatcher::MarkTableModified (static::GetDatabase()->Database, 'qualification');
		}
	}
	...
	*/
	protected static function BroadcastDeleteAll() {
		...
		public static function GetDatabaseName() {
			return QApplication::$Database[Qualification::GetDatabaseIndex()]->Database;
		}
		 ...
		 * any query calls (like Load or QueryArray), with a call to Qualification::ClearCache()
		 *
     ...
     **/
	class QQNodeQualification extends QQTableNode {
