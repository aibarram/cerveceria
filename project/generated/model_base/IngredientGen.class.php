<?php
	 ...
	 */
	class IngredientGen extends QBaseClass implements IteratorAggregate, JsonSerializable {
		...
		/** @var Ingredient[] Short term cached Ingredient objects */
	...
	///////////////////////////////////////////////////////////////////////
		...
		/**
		 * Protected member variable that maps to the database column ingredient.description
		...
		/**
		 * Protected member variable that maps to the database column ingredient.active
		...
		/**
		 * Construct a new Ingredient object.
		 */
		...
		{
			$this->strName = Ingredient::NameDefault;
			$this->__blnValid[self::NAME_FIELD] = true;
			$this->strDescription = Ingredient::DescriptionDefault;
			$this->__blnValid[self::DESCRIPTION_FIELD] = true;
			$this->blnActive = Ingredient::ActiveDefault;
		...
		**/
		...
		public static function Load($intId, $objOptionalClauses = null) {
			...
			$objToReturn = Ingredient::QuerySingle(
				QQ::AndCondition(
		...
		/**
		 * Static Qcubed Query method to query for a single Ingredient object.
		...
		/**
		 * Static Qcubed Query method to query for an array of Ingredient objects.
		 * Offloads work to QModelTrait.trait.php
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClausees additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		...
		public static function QueryPrimaryKeys(QQCondition $objConditions = null) {
			...
			$clauses[] = QQ::Select(QQN::Ingredient()->Id);
			$objIngredients = self::QueryArray($objConditions, $clauses);
			$pks = [];
			foreach ($objIngredients as $objIngredient) {
				$pks[] = $objIngredient->intId;
			}
			return $pks;
		}
		 ...
		 * @return mixed Either a Ingredient, or false to indicate the dbrow was used in an expansion, or null to indicate that this leaf is a duplicate.
		*/
		public static function InstantiateDbRow(
			...
			if (empty($objToReturn)) {
				...
				$objToReturn = new Ingredient(false);
		...
		public static function InstantiateDbResult(QDatabaseResultBase $objDbResult, $objExpandAsArrayNode = null, $strColumnAliasArray = null) {
			...
			if ($objExpandAsArrayNode) {
				...
				while ($objDbRow = $objDbResult->GetNextRow()) {
					$objItem = Ingredient::InstantiateDbRow($objDbRow, null, $objExpandAsArrayNode, $objPrevItemArray, $strColumnAliasArray);
					if ($objItem) {
						$objToReturn[] = $objItem;
		...
		public static function InstantiateCursor(QDatabaseResultBase $objDbResult) {
			...
			return Ingredient::InstantiateDbRow($objDbRow, null, null, null, $strColumnAliasArray);
		}
	...
	*/
	...
	public function Reload($clauses = null) {
		...
		// Reload the Object
		$objReloaded = Ingredient::Load($this->intId, $clauses);
		...
		/**
		 *  Return an array of Ingredients keyed by the unique Id property.
		 *	@param Ingredient[]
		 *	@return Ingredient[]
		 **/
		public static function KeyIngredientsById($ingredients) {
			if (empty($ingredients)) {
				return $ingredients;
			}
			foreach ($ingredients as $ingredient) {
				$aOut[$ingredient->intId] = $ingredient;
			}
			return $aOut;
		}
		...
		/**
		 *  Return an array of Ingredients keyed by the unique Name property.
		 *	@param Ingredient[]
		 *	@return Ingredient[]
		 **/
		public static function KeyIngredientsByName($ingredients) {
			if (empty($ingredients)) {
				return $ingredients;
			}
			foreach ($ingredients as $ingredient) {
				$aOut[$ingredient->strName] = $ingredient;
			}
			return $aOut;
		}
	...
	*/
	protected static function BroadcastInsert($pk) {
		if (static::$blnWatchChanges) {
			QWatcher::MarkTableModified (static::GetDatabase()->Database, 'ingredient');
		}
	}
	...
	*/
	protected static function BroadcastDeleteAll() {
		...
		public static function GetDatabaseName() {
			return QApplication::$Database[Ingredient::GetDatabaseIndex()]->Database;
		}
		 ...
		 * any query calls (like Load or QueryArray), with a call to Ingredient::ClearCache()
		 *
     ...
     **/
	class QQNodeIngredient extends QQTableNode {
