<?php
	/**
	 * The abstract BreweryGen class defined here is
	 * code-generated and contains all the basic CRUD-type functionality as well as
	 * basic methods to handle relationships and index-based loading.
	 *
	 * To use, you should use the Brewery subclass which
	 * extends this BreweryGen class.
	 *
	 * Because subsequent re-code generations will overwrite any changes to this
	 * file, you should leave this file unaltered to prevent yourself from losing
	 * any information or code changes.  All customizations should be done by
	 * overriding existing or implementing new methods, properties and variables
	 * in the Brewery class.
	 *
	 ...
	 */
	class BreweryGen extends QBaseClass implements IteratorAggregate, JsonSerializable {
		...
		/** @var Brewery[] Short term cached Brewery objects */
	...
	///////////////////////////////////////////////////////////////////////
		...
		/**
		 * Protected member variable that maps to the database PK Identity column brewery.id
		 * @var integer intId
		 */
		private $intId;
		...
		/**
		 * Protected member variable that maps to the database column brewery.name
		 * @var string strName
		 */
		private $strName;
		...
		/**
		 * Protected member variable that maps to the database column brewery.description
		...
		/**
		 * Protected member variable that maps to the database column brewery.active
		...
		/**
		 * Construct a new Brewery object.
		 */
		...
		{
			$this->strName = Brewery::NameDefault;
			$this->__blnValid[self::NAME_FIELD] = true;
			$this->strDescription = Brewery::DescriptionDefault;
			$this->__blnValid[self::DESCRIPTION_FIELD] = true;
			$this->blnActive = Brewery::ActiveDefault;
		...
		**/
		...
		public static function Load($intId, $objOptionalClauses = null) {
			...
			$objToReturn = Brewery::QuerySingle(
				QQ::AndCondition(
					QQ::Equal(QQN::Brewery()->Id, $intId)
				),
				$objOptionalClauses
			);
			return $objToReturn;
		...
		public static function LoadAll($objOptionalClauses = null) {
			...
			// Call Brewery::QueryArray to perform the LoadAll query
			try {
				return Brewery::QueryArray(QQ::All(), $objOptionalClauses);
		...
		public static function CountAll() {
			// Call Brewery::QueryCount to perform the CountAll query
			return Brewery::QueryCount(QQ::All());
		}

		
		/**
		 * Static Qcubed Query method to query for a single Brewery object.
		 * Offloads work to QModelTrait.trait.php
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClausees additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		 * @return Brewery the queried object
		 */
		...
		/**
		 * Static Qcubed Query method to query for an array of Brewery objects.
		 * Offloads work to QModelTrait.trait.php
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClausees additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		 * @return Brewery[] the queried objects as an array
		 */
		...
		public static function QueryPrimaryKeys(QQCondition $objConditions = null) {
			...
			$clauses[] = QQ::Select(QQN::Brewery()->Id);
			$objBreweries = self::QueryArray($objConditions, $clauses);
			$pks = [];
			foreach ($objBreweries as $objBrewery) {
				$pks[] = $objBrewery->intId;
			}
			return $pks;
		}
		...
		/**
		 * Instantiate a Brewery from a Database Row.
		 * Takes in an optional strAliasPrefix, used in case another Object::InstantiateDbRow
		 * is calling this Brewery::InstantiateDbRow in order to perform
		 ...
		 * @return mixed Either a Brewery, or false to indicate the dbrow was used in an expansion, or null to indicate that this leaf is a duplicate.
		*/
		public static function InstantiateDbRow(
			...
			if (empty($objToReturn)) {
				// Create a new instance of the Brewery object
				$objToReturn = new Brewery(false);
		...
		/**
		 * Instantiate an array of Breweries from a Database Result
		...
		public static function InstantiateDbResult(QDatabaseResultBase $objDbResult, $objExpandAsArrayNode = null, $strColumnAliasArray = null) {
			...
			if ($objExpandAsArrayNode) {
				...
				while ($objDbRow = $objDbResult->GetNextRow()) {
					$objItem = Brewery::InstantiateDbRow($objDbRow, null, $objExpandAsArrayNode, $objPrevItemArray, $strColumnAliasArray);
					if ($objItem) {
						$objToReturn[] = $objItem;
				...
				while ($objDbRow = $objDbResult->GetNextRow())
					$objToReturn[] = Brewery::InstantiateDbRow($objDbRow, null, null, null, $strColumnAliasArray);
			}
		...
		/**
		 * Instantiate a single Brewery object from a query cursor (e.g. a DB ResultSet).
		 * Cursor is automatically moved to the "next row" of the result set.
		 * Will return NULL if no cursor or if the cursor has no more rows in the resultset.
		 * @param QDatabaseResultBase $objDbResult cursor resource
		 * @return Brewery next row resulting from the query
		 */
		public static function InstantiateCursor(QDatabaseResultBase $objDbResult) {
			...
			return Brewery::InstantiateDbRow($objDbRow, null, null, null, $strColumnAliasArray);
		}
		...
		*/
		...
		public function Save($blnForceInsert = false, $blnForceUpdate = false) {
			...
			$objDatabase = Brewery::GetDatabase();
	...
	*/
	protected function Insert() {
		...
		$mixToReturn = $this->intId = $objDatabase->InsertId('brewery', 'id');
	...
	protected function GetValueClause() {
		...
		public function Delete() {
			if ((is_null($this->intId)))
				throw new QUndefinedPrimaryKeyException('Cannot delete this Brewery with an unset primary key.');
		...
		/**
	 * Reload this Brewery from the database.
	 * @return void
	 */
	public function Reload($clauses = null) {
		// Make sure we are actually Restored from the database
		if (!$this->__blnRestored)
			throw new QCallerException('Cannot call Reload() on a new, unsaved Brewery object.');
		...
		// Reload the Object
		$objReloaded = Brewery::Load($this->intId, $clauses);
		...
		/**
		 *  Return an array of Breweries keyed by the unique Id property.
		 *	@param Brewery[]
		 *	@return Brewery[]
		 **/
		public static function KeyBreweriesById($breweries) {
			if (empty($breweries)) {
				return $breweries;
			}
			foreach ($breweries as $brewery) {
				$aOut[$brewery->intId] = $brewery;
			}
			return $aOut;
		}
	...
	*/
	protected static function BroadcastInsert($pk) {
		if (static::$blnWatchChanges) {
			QWatcher::MarkTableModified (static::GetDatabase()->Database, 'brewery');
		}
	}
	...
	*/
	protected static function BroadcastDeleteAll() {
		...
		public static function GetDatabaseName() {
			return QApplication::$Database[Brewery::GetDatabaseIndex()]->Database;
		}
		...
		public static function GetSoapComplexTypeXml() {
			$strToReturn = '<complexType name="Brewery"><sequence>';
		...
		public static function AlterSoapComplexTypeArray(&$strComplexTypeArray) {
			if (!array_key_exists('Brewery', $strComplexTypeArray)) {
				$strComplexTypeArray['Brewery'] = Brewery::GetSoapComplexTypeXml();
			}
		}
		...
		public static function GetArrayFromSoapArray($objSoapArray) {
			...
			foreach ($objSoapArray as $objSoapObject)
				array_push($objArrayToReturn, Brewery::GetObjectFromSoapObject($objSoapObject));

			return $objArrayToReturn;
		}

		public static function GetObjectFromSoapObject($objSoapObject) {
			$objToReturn = new Brewery();
		...
		public static function GetSoapArrayFromArray($objArray) {
			...
			foreach ($objArray as $objObject)
				array_push($objArrayToReturn, Brewery::GetSoapObjectFromObject($objObject, true));
		 ...
		 * any query calls (like Load or QueryArray), with a call to Brewery::ClearCache()
		 *
     ...
     **/
	class QQNodeBrewery extends QQTableNode {
		protected $strTableName = 'brewery';
		protected $strPrimaryKey = 'id';
		protected $strClassName = 'Brewery';
     ...
     * @property-read QQNode $_PrimaryKeyNode
     **/
	class QQReverseReferenceNodeBrewery extends QQReverseReferenceNode {
		protected $strTableName = 'brewery';
		protected $strPrimaryKey = 'id';
		protected $strClassName = 'Brewery';
