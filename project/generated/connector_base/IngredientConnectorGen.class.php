<?php
	 ...
	 * must create a new QForm or QPanel which instantiates a IngredientConnector
	 * class.
	 *
	...
	class IngredientConnectorGen extends QBaseClass {
		// General Variables
		/**
		...
		// Controls that correspond to Ingredient's individual data fields
		/**
		 * @var QLabel lblId
		 * @access protected
		 */
		 ...
		 * IngredientConnector to edit a single Ingredient object within the
		 * QPanel or QForm.
		 *
		 * This constructor takes in a single Ingredient object, while any of the static
		 * create methods below can be used to construct based off of individual PK ID(s).
		 *
		 * @param mixed $objParentObject QForm or QPanel which will be using this IngredientConnector
		 ...
		 */
		 public function __construct($objParentObject, Ingredient $objIngredient) {
			// Setup Parent Object (e.g. QForm or QPanel which will be using this IngredientConnector)
		 ...
		 * @param integer $intCreateType rules governing Ingredient object creation - defaults to CreateOrEdit
 		 * @return IngredientConnector
		...
		public static function Create($objParentObject, $intId = null, $intCreateType = QModelConnectorCreateType::CreateOrEdit) {
			...
			if (strlen($intId)) {
				...
				if ($objIngredient)
					return new IngredientConnector($objParentObject, $objIngredient);
			...
			return new IngredientConnector($objParentObject, new Ingredient());
		}
		...
		public static function CreateFromPathInfo($objParentObject, $intCreateType = QModelConnectorCreateType::CreateOrEdit) {
			...
			return IngredientConnector::Create($objParentObject, $intId, $intCreateType);
		}
		...
		public function lblId_Create($strControlId = null) {
			...
			$this->lblId->LinkedNode = QQN::Ingredient()->Id;
			$this->lblId->Text =  $this->blnEditMode ? $this->objIngredient->Id : QApplication::Translate('N\A');
			return $this->lblId;
		}
		...
		public function txtName_Create($strControlId = null) {
			...
			$this->txtName->LinkedNode = QQN::Ingredient()->Name;
		...
		public function txtDescription_Create($strControlId = null) {
			...
			$this->txtDescription->MaxLength = Ingredient::DescriptionMaxLength;
			$this->txtDescription->PreferredRenderMethod = 'RenderWithName';
			$this->txtDescription->LinkedNode = QQN::Ingredient()->Description;
			$this->txtDescription->Text = $this->objIngredient->Description;
		...
		public function lblDescription_Create($strControlId = null) {
			...
			$this->lblDescription->LinkedNode = QQN::Ingredient()->Description;
			$this->lblDescription->Text = $this->objIngredient->Description;
		...
		public function chkActive_Create($strControlId = null) {
			...
			$this->chkActive->Checked = $this->objIngredient->Active;
			$this->chkActive->PreferredRenderMethod = 'RenderWithName';
			$this->chkActive->LinkedNode = QQN::Ingredient()->Active;
		...
		public function lblActive_Create($strControlId = null) {
			...
			$this->lblActive->LinkedNode = QQN::Ingredient()->Active;
			$this->lblActive->Text = $this->objIngredient->Active ? QApplication::Translate('Yes') : QApplication::Translate('No');
		...
		/**
		 * Refresh this ModelConnector with Data from the local Ingredient object.
		 * @param boolean $blnReload reload Ingredient from the database
		 * @return void
		 */
		public function Refresh($blnReload = false) {
			assert($this->objIngredient); // Notify in development version
			if (!($this->objIngredient)) return; // Quietly fail in production

			if ($blnReload) {
				$this->objIngredient->Reload();
			}
			if ($this->lblId) $this->lblId->Text =  $this->blnEditMode ? $this->objIngredient->Id : QApplication::Translate('N\A');
			...
			if ($this->txtDescription) $this->txtDescription->Text = $this->objIngredient->Description;
			if ($this->lblDescription) $this->lblDescription->Text = $this->objIngredient->Description;


			if ($this->chkActive) $this->chkActive->Checked = $this->objIngredient->Active;
			if ($this->lblActive) $this->lblActive->Text = $this->objIngredient->Active ? QApplication::Translate('Yes') : QApplication::Translate('No');


		}

		/**
		 * Load this ModelConnector with a Ingredient object. Returns the object found, or null if not
		 ...
		 */
		 public function Load($intId = null, $objClauses = null) {
			if (!is_null($intId)) {
				$this->objIngredient = Ingredient::Load($intId, $objClauses);
				$this->strTitleVerb = QApplication::Translate('Edit');
				$this->blnEditMode = true;
			}
			else {
				$this->objIngredient = new Ingredient();
				$this->strTitleVerb = QApplication::Translate('Create');
				$this->blnEditMode = false;
			}
			if ($this->objIngredient) {
				$this->Refresh ();
			}
		...
		/**
		* This will update this object's Ingredient instance,
		...
		*/
		public function UpdateIngredient() {
			try {
				...
				if ($this->txtDescription) $this->objIngredient->Description = $this->txtDescription->Text;

				if ($this->chkActive) $this->objIngredient->Active = $this->chkActive->Checked;
		 ...
		 * updating only the fields which have had a control created for it.
		 */
		public function SaveIngredient($blnForceUpdate = false) {
			try {
				$this->UpdateIngredient();
                $id = $this->objIngredient->Save(false, $blnForceUpdate);
